...
test('catalog labels repurposed starters by their canonical algorithms', () => {
  const expectedNames = {
    ...
    'spiral-matrix-traversal': 'Spiral Traversal',
    'rotate-matrix': 'Matrix Rotation',
    'matrix-transpose': 'Matrix Zeroing',
    'pre-order-traversal': 'Binary Search Tree Validation',
    'post-order-traversal': 'Level-Order Traversal (BFS)',
    'postfix-evaluation': 'Monotonic Stack',
    'breadth-first-search': "Topological Sort (Kahn's Algorithm)",
    'reverse-queue': 'Grid Breadth-First Search (BFS)',
  } as const satisfies Partial<Record<AlgorithmId, string>>;

  for (const [id, expectedName] of Object.entries(expectedNames)) {
    assert.equal(
      algorithmCatalog.find((algorithm) => algorithm.id === id)?.name,
      expectedName,
      `${id} must identify the algorithm implemented by its starter.`,
    );
  }
});
