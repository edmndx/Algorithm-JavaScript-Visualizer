...
type StackCompare = {
  ...
  readonly target: BinaryExpression;
  readonly peek: StackPeek;
  readonly sourceOperator: '===' | '!==' | '<' | '<=' | '>' | '>=';
  readonly traceOperator: StackComparisonOperator;
  readonly topOnLeft: boolean;
  readonly line: number;
};

type StackOperation = StackCall | StackPeek | StackCompare;
...
export function instrumentStack(
  ...
  const peekHelper = candidate.operations.some(({ kind }) => kind === 'peek')
    ? allocateIdentifier()
    : null;
  const compareHelper = candidate.operations.some(
    ({ kind }) => kind === 'compare',
  )
    ? allocateIdentifier()
    : null;
  ...
  for (const operation of candidate.operations) {
    ...
    operationEdits.push({
      start: stackOperationNode(operation).start,
      end: stackOperationNode(operation).end,
      text: replacement,
    });
  }
...
function renderStackHelpers(
  ...
  return (
    ...
    (compare === null
      ? ''
      : `const ${compare} = (left, right, sourceOperator, traceOperator, topOnLeft, line) => { const top = topOnLeft ? left : right; const value = topOnLeft ? right : left; const result = sourceOperator === '===' ? left === right : sourceOperator === '!==' ? left !== right : sourceOperator === '<' ? left < right : sourceOperator === '<=' ? left <= right : sourceOperator === '>' ? left > right : left >= right; if (${isTraceValue}(top) && ${isTraceValue}(value)) trace.compare({ value, operator: traceOperator, source: { line } }); return result; };\n`)
  );
}
...
function analyzeStack(
  ...
  const comparedPeekTargets = new Set<StackPeek['target']>();
  ...
  walkAst(
    ...
    (node, parent, _grandparent, insideUnsupportedScope) => {
      ...
      const comparison = matchStackComparison(node, root);
      if (comparison !== null) {
        comparedPeekTargets.add(comparison.peek.target);
        operations.push(comparison);
        return;
      }

      const peek = matchStackPeek(node, parent, root);
      if (peek !== null && !comparedPeekTargets.has(peek.target)) {
  ...
  if (
    !operations.some(
      ({ kind }) => kind === 'pop' || kind === 'peek' || kind === 'compare',
    ) ||
    operations.some(
      (operation) => stackOperationNode(operation).start < declaration.end,
    ) ||
...
function matchStackComparison(
  ...
  const leftPeek = matchStackPeek(node.left, node, root);
  const rightPeek = matchStackPeek(node.right, node, root);
  if ((leftPeek === null) === (rightPeek === null)) return null;

  const topOnLeft = leftPeek !== null;
  const peek = leftPeek ?? rightPeek;
  const line = sourceLine(node);
  if (peek === null || line === null) return null;
  ...
  return {
    ...
    traceOperator: normalizeComparisonOperator(node.operator, topOnLeft),
    topOnLeft,
    line,
  };
}
...
function normalizeComparisonOperator(
  operator: StackCompare['sourceOperator'],
  topOnLeft: boolean,
  ...
  switch (operator) {
    ...
    case '!==':
      return 'neq';
    case '<':
      return topOnLeft ? 'lt' : 'gt';
    case '<=':
      return topOnLeft ? 'lte' : 'gte';
    case '>':
      return topOnLeft ? 'gt' : 'lt';
    case '>=':
      return topOnLeft ? 'gte' : 'lte';
  }
}
...
function hasUnsafeStackUsage(
  ...
  const supportedCalls = new Set(
    operations.flatMap((operation) => {
      switch (operation.kind) {
        ...
        case 'peek':
        case 'compare': {
          const peek = operation.kind === 'peek' ? operation : operation.peek;
          return peek.target.type === 'CallExpression' ? [peek.target] : [];
        }
      }
    }),
  );
  const supportedMembers = new Set(
    operations.flatMap((operation) =>
      operation.kind === 'peek' || operation.kind === 'compare'
        ? (operation.kind === 'peek' ? operation : operation.peek).members
        : [operation.member],
    ),
  );
  let unsafe = false;
...
function stackOperationReplacement(
  ...
  switch (operation.kind) {
    ...
    case 'compare':
      return compareHelper === null
        ? null
        : `${compareHelper}(${source.slice(operation.target.left.start, operation.target.left.end)}, ${source.slice(operation.target.right.start, operation.target.right.end)}, ${JSON.stringify(operation.sourceOperator)}, ${JSON.stringify(operation.traceOperator)}, ${String(operation.topOnLeft)}, ${operation.line})`;
  }
}
