...
export function reduceHashTable(
  ...
  switch (command.type) {
    ...
    case 'hash-table.probe':
      requireEntity(
        command.matchedEntryId === null ||
          scene.entries.some((entry) => entry.id === command.matchedEntryId),
        `Hash-table entry "${command.matchedEntryId}" does not exist.`,
      );
      return {
        ...scene,
        probe: {
          ...
          matchedEntryId: command.matchedEntryId,
        },
      };
