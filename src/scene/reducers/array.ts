    ...
    case 'array.focus':
      return {
        ...
        focus: {
          ...
          pointers:
            command.pointers === undefined ? {} : { ...command.pointers },
          range: command.range === undefined ? null : [...command.range],
        },
      };

    case 'array.compareValue': {
      assertIndex(command.index, scene.values.length, command.type);
      const actual = scene.values[command.index];
      if (actual === undefined)
        throw new SceneReducerError(
          'INDEX_OUT_OF_BOUNDS',
          'Array value is missing.',
        );
      return {
        ...scene,
        valueComparison: {
          ...
          matches: compare(actual, command.value, command.operator),
        },
      };
    }
